...
"""
VGGT-Omega + Bundle Adjustment Integration Script

This script integrates VGGT-Omega (faster, more flexible resolution)
with the Bundle Adjustment optimization pipeline from the original VGGT.

Key differences from demo_colmap.py:
1. Uses VGGT-Omega instead of VGGT (flexible input resolution)
2. Automatically converts FOV encoding to standard camera matrices
3. All other BA pipeline components remain identical
4. Supports both feedforward and BA-optimized modes
...
Usage:
    # VGGT mode (original)
    python demo_colmap_omega.py --model vggt --scene-dir /path/to/scene --use-ba
    
    # VGGT-Omega mode (new)
    python demo_colmap_omega.py --model vggt-omega \\
        --omega-checkpoint /path/to/checkpoint.pt \\
...
# ========================================================================================
# VGGT-Omega Imports (Optional)
# ========================================================================================
try:
    from vggt_omega.models import VGGTOmega
    from vggt_omega.utils.load_fn import load_and_preprocess_images as omega_load_and_preprocess_images
    from vggt_omega.utils.pose_enc import encoding_to_camera as omega_encoding_to_camera
    OMEGA_AVAILABLE = True
except ImportError:
    OMEGA_AVAILABLE = False
    print("⚠️  VGGT-Omega not installed. Install with: pip install git+https://github.com/facebookresearch/vggt-omega.git")
...
def prepare_bae(
    ...
    """
    Convert Batched NumPy Arrays to PyCOLMAP and run Bundle Adjustment
...
def parse_args():
    parser = argparse.ArgumentParser(description="VGGT + VGGT-Omega + BA Demo")
    
    # Model selection
    parser.add_argument("--model", type=str, default="vggt", choices=["vggt", "vggt-omega"],
                        help="Choose model: vggt (original) or vggt-omega (faster, flexible resolution)")
    
    # VGGT-Omega specific
    parser.add_argument("--omega-checkpoint", type=str, default=None,
                        help="Path to VGGT-Omega checkpoint (required for vggt-omega mode)")
    parser.add_argument("--image-resolution", type=int, default=512,
                        help="Input image resolution for VGGT-Omega (default: 512)")
    
    # Scene and seed
    parser.add_argument("--scene-dir", type=str, required=True, help="Directory containing the scene images")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for reproducibility")
    
    # BA parameters
    parser.add_argument("--use-ba", action="store_true", default=False, help="Use BA for reconstruction")
    parser.add_argument("--max-reproj-error", type=float, default=8.0,
                        help="Maximum reprojection error for reconstruction")
    parser.add_argument("--implementation", type=str, default="pycolmap", 
                        choices=["pycolmap", "bae"], help="BA implementation (default: pycolmap)")
    ...
    parser.add_argument("--camera-type", type=str, default="SIMPLE_PINHOLE",
                        help="Camera type for reconstruction (default: SIMPLE_PINHOLE)")
    parser.add_argument("--vis-thresh", type=float, default=0.2, help="Visibility threshold for tracks (default: 0.2)")
    parser.add_argument("--query-frame-num", type=int, default=8, help="Number of frames to query (default: 8)")
    parser.add_argument("--max-query-pts", type=int, default=4096, help="Maximum number of query points (default: 4096)")
    ...
    parser.add_argument("--conf-thres-value", type=float, default=5.0,
                        help="Confidence threshold value for depth filtering (default: 5.0)")
    
    return parser.parse_args()


def run_vggt_omega(model, images, dtype, image_resolution=512):
    """
    Run VGGT-Omega inference
    
    Args:
        model: VGGTOmega model instance
        images: Input images [B, 3, H, W]
        dtype: Data type for inference
        image_resolution: Input image resolution
    
    Returns:
        extrinsic: [N, 3, 4] camera extrinsic matrices
        intrinsic: [N, 3, 3] camera intrinsic matrices
        depth_map: [N, H, W, 1] depth maps
        depth_conf: [N, H, W, 1] depth confidence
    """
    ...
    # ▶ Convert FOV encoding to standard camera matrices
    extrinsic, intrinsic = omega_encoding_to_camera(
        predictions["pose_enc"],
        predictions["images"].shape[-2:],  # Get actual inference resolution
    )
    
    depth_map = predictions["depth"]
    depth_conf = predictions["depth_conf"]
    
    return extrinsic.cpu().numpy(), intrinsic.cpu().numpy(), depth_map.cpu().numpy(), depth_conf.cpu().numpy()


def run_VGGT(model, images, dtype, resolution=518):
    """Original VGGT inference (kept for compatibility)"""
    # images: [B, 3, H, W]
...
def demo_fn(args):
    # Print configuration
    print("\n" + "=" * 80)
    print("VGGT / VGGT-Omega + Bundle Adjustment Demo")
    print("=" * 80)
    print(f"Model: {args.model.upper()}")
    print(f"Scene: {args.scene_dir}")
    print(f"BA: {'Enabled' if args.use_ba else 'Disabled'}")
    print("=" * 80 + "\n")
    ...
    # ==================== Load Model ====================
    print(f"\n[1/6] Loading {args.model.upper()} model...")
    
    if args.model == "vggt-omega":
        if not OMEGA_AVAILABLE:
            raise RuntimeError("VGGT-Omega not installed. Install with: pip install git+https://github.com/facebookresearch/vggt-omega.git")
        
        if not args.omega_checkpoint:
            raise ValueError("--omega-checkpoint is required for VGGT-Omega mode")
        
        if not os.path.isfile(args.omega_checkpoint):
            raise FileNotFoundError(f"Checkpoint not found: {args.omega_checkpoint}")
        
        model = VGGTOmega().eval()
        model.load_state_dict(torch.load(args.omega_checkpoint, map_location="cpu"))
        model = model.to(device)
        print(f"✓ VGGT-Omega model loaded from {args.omega_checkpoint}")
        inference_fn = lambda imgs, dtype: run_vggt_omega(model, imgs, dtype, args.image_resolution)
        ...
        model.eval()
        model = model.to(device)
        print(f"✓ VGGT-1B model loaded")
        inference_fn = lambda imgs, dtype: run_VGGT(model, imgs, dtype)
    ...
    # ==================== Prepare images ====================
    if args.model == "vggt-omega":
        # Flexible resolution for Omega
        images = omega_load_and_preprocess_images(image_path_list, image_resolution=args.image_resolution)
        img_load_resolution = None  # Omega handles its own resolution
        vggt_fixed_resolution = args.image_resolution
        original_coords = None
    else:
        # Fixed resolution for original VGGT
    ...
    # ==================== Run inference ====================
    print(f"\n[3/6] Running {args.model.upper()} inference...")
    extrinsic, intrinsic, depth_map, depth_conf = inference_fn(images, dtype)
    ...
    # ==================== Unproject to 3D ====================
    print(f"\n[4/6] Unprojecting depth to 3D points...")
    points_3d = unproject_depth_map_to_point_map(depth_map, extrinsic, intrinsic)
    print(f"✓ Points 3D shape: {points_3d.shape}")

    # ==================== Bundle Adjustment Pipeline ====================
    if args.use_ba:
        print(f"\n[5/6] Running Bundle Adjustment pipeline...")
        
        image_size = np.array(images.shape[-2:])
        if img_load_resolution is not None:
        ...
        with torch.cuda.amp.autocast(dtype=dtype):
            # Predicting Tracks
            print(f"   [5a] Predicting tracks...")
            pred_tracks, pred_vis_scores, pred_confs, points_3d, points_rgb = predict_tracks(
                images,
                conf=torch.tensor(depth_conf, device=device),
                points_3d=torch.tensor(points_3d, device=device),
                masks=None,
        ...
        # Rescale intrinsic matrix if needed
        if img_load_resolution is not None:
            intrinsic[:, :2, :] *= scale
        ...
        if args.implementation == "bae":
            print(f"   [5b] Running BAE (Bundle Adjustment Eager) optimization...")
            prepare_bae(
                points_3d.cpu().numpy(),
                extrinsic,
                intrinsic,
                ...
                image_size,
                masks=track_mask.cpu().numpy(),
                max_reproj_error=args.max_reproj_error,
                shared_camera=shared_camera,
                camera_type=args.camera_type,
                points_rgb=points_rgb.cpu().numpy() if isinstance(points_rgb, torch.Tensor) else points_rgb,
            )
            print(f"   ✓ BAE optimization complete")

        # Convert to COLMAP format
        print(f"   [5c] Converting to COLMAP format...")
        reconstruction, valid_track_mask = batch_np_matrix_to_pycolmap(
            points_3d.cpu().numpy() if isinstance(points_3d, torch.Tensor) else points_3d,
            extrinsic,
            intrinsic,
            pred_tracks.cpu().numpy() if isinstance(pred_tracks, torch.Tensor) else pred_tracks,
            image_size,
            masks=track_mask.cpu().numpy() if isinstance(track_mask, torch.Tensor) else track_mask,
            max_reproj_error=args.max_reproj_error,
            shared_camera=shared_camera,
            camera_type=args.camera_type,
            points_rgb=points_rgb.cpu().numpy() if isinstance(points_rgb, torch.Tensor) else points_rgb,
        ...
        # PyColmap Bundle Adjustment (if not using BAE)
        if not args.implementation == "bae":
            print(f"   [5d] Running PyColmap BA...")
            ba_options = pycolmap.BundleAdjustmentOptions()
            pycolmap.bundle_adjustment(reconstruction, ba_options)
            print(f"   ✓ PyColmap BA complete")

        reconstruction_resolution = img_load_resolution if img_load_resolution else vggt_fixed_resolution
        
    else:
        # Feedforward mode (no BA)
        print(f"\n[5/6] Feedforward mode (no BA)...")
    ...
    points_3d_export = points_3d.cpu().numpy() if isinstance(points_3d, torch.Tensor) else points_3d
    trimesh.PointCloud(points_3d_export, colors=points_rgb).export(os.path.join(args.scene_dir, "sparse/points.ply"))
